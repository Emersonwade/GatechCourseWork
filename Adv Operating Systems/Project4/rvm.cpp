 ...
 */
 ...
 void *rvm_map(rvm_t rvm, const char *segname, int size_to_create) {
	...
	if(fileExists(seg_path_cstr)) {
		...
		fclose(fp);
		rvm_replaylog(rvm, segname, segment);
	} 
	else {
...
void rvm_truncate_log(rvm_t rvm) {
	...
	while(!feof(fp)) {
		...
		fread(&r_txender, sizeof(trans_end_t), 1, fp);
		...
		for (size_t i = 0; i < record_list.size(); i++) {
			char *segment_file_path = new char [strlen(rvm.directory) + strlen(record_list[i]->segmentname) + 5];
			...
			fseek(backupFile, record_list[i]->region_offset, SEEK_CUR);
			fwrite((char*)record_list[i]->data, record_list[i]->region_size, 1, backupFile);
			fclose(backupFile);
			delete[] segment_file_path;
		}
		std::cout << "c" << std::endl;
