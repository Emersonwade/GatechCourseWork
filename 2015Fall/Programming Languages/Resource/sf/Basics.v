...
Definition blt_nat (n m : nat) : bool := negb (nandb (ble_nat n m)  (negb (beq_nat n m))).
...
Eval compute in incr (incr (incr B)).
...
Fixpoint bin_to_nat (b : bin) : nat :=
  match b with
  | B => O
  | TB b' => doublenat (bin_to_nat b')
  | OTB b' => S ( doublenat (bin_to_nat b'))
  end.

Example test_bin_incr1:     incr (incr (incr (incr B))) = TB (TB (OTB B)).
Proof. reflexivity.  Qed.

Example test_bin_incr2:     bin_to_nat (OTB (TB (TB (OTB B)))) = 9.
Proof. reflexivity.  Qed.

Example test_bin_incr3:     bin_to_nat (incr (incr (incr (incr B)))) = 4.
Proof. reflexivity.  Qed.

Example test_bin_incr4:     bin_to_nat (incr (TB (TB (incr B)))) = 5.
Proof. reflexivity.  Qed.

Example test_bin_incr5:     S (bin_to_nat (OTB (TB (TB (OTB B))))) = bin_to_nat (incr (OTB (TB (TB (OTB B))))).
Proof. reflexivity.  Qed.
